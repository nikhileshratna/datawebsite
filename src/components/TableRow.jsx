...
const TableRow = ({ tableId, rowId, row }) => {
  ...
  return (
    <>
      <tr onClick={() => toggleRow(tableId, rowId)} className="cursor-pointer min-w-[100%]">
      ...
      {isRowCollapsed && row.details && (
        <tr className="bg-[#ecfdf5]">
          <td colSpan={row.data.length} className="py-2 px-4">
            <table className="w-full table-fixed" style={{ borderSpacing: '0 10px' }}> {/* Added borderSpacing */}
              <thead className="py-2 px-4 border-b border-gray-300 cursor-pointer">
                <tr>
                  {row.details.columns.map((column, columnIndex) => (
                    <th key={columnIndex} className="font-semibold py-2 px-4">{column}</th>
                  ))}
                </tr>
              </thead>
              <tbody>
                <tr>
                  {row.details.data.map((detail, detailIndex) => (
                    <td key={detailIndex} className="py-2 px-4">{detail}</td>
                  ))}
                </tr>
              </tbody>
            </table>
